...
//    argv = const_cast<char**>(newArgv);
    ...
    QString directoryPath = QString::fromLocal8Bit(argv[1]);
    QDir dir(directoryPath);

    if (!dir.exists())
    {
        err << "Error: Directory does not exist: " << directoryPath << "\n";
        return 1;
    }
    ...
    // Расчёт
    IvanCornelius_FilesChecksum calculator(directoryPath);
    calculator.start();
